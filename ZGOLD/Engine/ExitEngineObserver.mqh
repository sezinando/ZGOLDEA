...
private:
 ...
 void EvalBasket(int d,int magic,bool &trig,double &profit,double &target){int n=0;profit=0;for(int i=OrdersTotal()-1;i>=0;i--){if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES))continue;if(OrderSymbol()!=Symbol()||OrderMagicNumber()!=magic)continue;int t=OrderType();if((d==OP_BUY&&t!=OP_BUY)||(d==OP_SELL&&t!=OP_SELL))continue;n++;profit+=OrderProfit()+OrderSwap()+OrderCommission();}target=n*ZGoldParams::StopProfit();trig=(n>0&&profit>=target);}
 void EvalComp(int d,int magic,double side,double opp,bool &trig,int &winT,int &l1T,int &l2T,double &winP,double &result){int tk[ZGOLD_EXIT_MAX_POS];double pf[ZGOLD_EXIT_MAX_POS];int n=0;trig=false;winT=l1T=l2T=-1;winP=0;result=0;for(int i=OrdersTotal()-1;i>=0&&n<ZGOLD_EXIT_MAX_POS;i--){if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES))continue;if(OrderSymbol()!=Symbol()||OrderMagicNumber()!=magic)continue;int t=OrderType();if((d==OP_BUY&&t!=OP_BUY)||(d==OP_SELL&&t!=OP_SELL))continue;tk[n]=OrderTicket();pf[n]=OrderProfit()+OrderSwap()+OrderCommission();n++;}if(n>ZGOLD_EXIT_MAX_POS-1)m_capacity_warning=true;if(n<=ZGoldParams::CompressionMinCount())return;int w=0;for(int j=1;j<n;j++)if(pf[j]>pf[w])w=j;int a=-1,b=-1;for(int k=0;k<n;k++){if(k==w)continue;if(a<0||pf[k]<pf[a]){b=a;a=k;}else if(b<0||pf[k]<pf[b])b=k;}if(a<0||b<0)return;double do32=pf[w]+pf[a]+pf[b];if(side>opp+ZGoldParams::CompressionLotMultiplier()*OrderLotsByTicket(tk[w],magic)&&do32>0){trig=true;winT=tk[w];l1T=tk[a];l2T=tk[b];winP=pf[w];result=do32;}}
