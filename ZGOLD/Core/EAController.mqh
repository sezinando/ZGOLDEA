...
#include "../Engine/CloseByObserver.mqh"
#include "../Engine/ExitDecisionObserver.mqh"
#include "../Engine/RecoveryCycleObserver.mqh"
class EAController
{
private:
 MarketState m_market; StateReconciler m_reconciler; DebugPanel m_panel; PendingTrailingObserver m_trailing; PendingTrailingDecisionObserver m_trailing_decision; OperationalState m_operational_state; GeometryObserver m_geometry; LayerEngineObserver m_layer; ExitEngineObserver m_exit; CloseByObserver m_closeby; ExitDecisionObserver m_exit_decision; RecoveryCycleObserver m_recovery; bool m_initialized; int m_magic;
 ...
 void UpdateExitObserver(ExposureState &e){m_exit.Evaluate(e,m_magic);m_closeby.Evaluate(m_magic);bool closeby_mode=m_exit.GlobalTriggered();m_exit_decision.Evaluate(m_exit,m_closeby,closeby_mode);m_recovery.Evaluate(m_magic,m_exit.GlobalTriggered());string text="NONE";string reason="NO EXIT TRIGGER";double value=m_exit.TotalProfit();double target=0.0;double result=m_exit.CompressionResult();int ticket=-1;int loss1=-1;int loss2=-1;if(m_exit.BasketTriggered()){text="BASKET";reason="DIRECTIONAL PROFIT >= COUNT*20";value=m_exit.BasketProfit();target=m_exit.BasketTarget();}else if(m_exit.CompressionTriggered()){text="COMPRESSION";reason="WINNER + 2 WORST | EXPOSURE GATE";value=m_exit.WinnerProfit();ticket=m_exit.WinnerTicket();loss1=m_exit.Loss1Ticket();loss2=m_exit.Loss2Ticket();}else if(m_exit.GlobalTriggered()){text="GLOBAL EXIT";reason="TOTAL PROFIT >= 4";}if(m_recovery.Status()!=ZGOLD_RECOVERY_NONE){Print("[ZGOLD][RECOVERY] ",m_recovery.Reason());}m_panel.SetExit(text,reason,value,target,result,ticket,loss1,loss2);m_panel.SetDecisionAction(m_exit_decision.Action(),m_exit_decision.Action(),m_exit_decision.Reason(),m_exit_decision.Ticket());}
